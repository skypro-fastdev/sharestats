...
from src.db.models import StudentProduct, ProductDB, StudentDB
...
async def process_purchase(session: AsyncSession, data: Purchase) -> StudentProduct:
    async with session.begin():
        ...
        query = (
            select(ProductDB)
            .where(ProductDB.id == data.product_id, ProductDB.is_active == True)
        )
        ...
        if not product:
            raise HTTPException(
                ...
                detail={
                    "status": "error",
                    "message": f"Продукт {data.product_id} не существует либо не активен"
                }
            )
        ...
        if not student:
            raise HTTPException(
                ...
                detail={
                    "status": "error",
                    "message": f"Студент с ID {data.student_id} не найден"
                }
            )
